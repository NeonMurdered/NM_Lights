class NM_CampFire_Empty extends NM_Fire_Base
{
	void NM_CampFire_Empty()
	{
		StartFireEffect();
		StartHeating();
		...
		if ( !GetGame().IsMultiplayer() || GetGame().IsClient() )
		{
			...
			m_CampfireLightEmpty = CampfireLightEmpty.Cast( ScriptedLightBase.CreateLight( CampfireLightEmpty, "0 0 0" ) );
			m_CampfireLightEmpty.AttachOnObject(this, m_ParticleLocalPos + Vector (0,-1,0.5));
	...
	void ~NM_CampFire_Empty()
	{
		...
		if (m_CampfireLightEmpty)
			m_CampfireLightEmpty.FadeOut();

		m_UTSource.SetActive(false);
		m_CampfireLightEmpty = NULL;
	}
...
class CampfireLightEmpty extends PointLightBase
{	
	void CampfireLightEmpty()
	{
